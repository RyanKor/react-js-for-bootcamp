...
- 블로그 회원가입, 로그인 기능을 구현하기 위해 상태 관리하는 과정(Redux, SAGA 등)을 초반에 설계부터 구체적으로 다뤘다. 다만, 서버사이드 렌더링을 별도로 추가해서 사용하지는 않았다. 심지어 Babel, Webpack에 대한 내용도 일절 다루지 않았고 React 개발 환경 수동 세팅하는 과정도 일절 다루지 않아서 상당히 문제가 되는 것 같다.
...
- 여기에 `Typescript`얹어서 사용하는 것 생각하면, 자바스크립트가 왜 실무에서 많이 쓰이는지 이해가 충분히 되는 것 같다.
